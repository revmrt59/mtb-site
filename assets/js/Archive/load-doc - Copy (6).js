...
function addScriptureExplainButtons(root, meta, scriptureDocPath) {
  ...
  function cleanNkJVClone(container, reference) {
    ...
    // ---------------------------------------
// LISTEN FOR SCRIPTURE "i" BUTTON CLICKS
// ---------------------------------------
if (!document.body.dataset.mtbExplainVerseBound) {

  document.addEventListener("mtb:explain-verse", function (e) {
    const verse =
      e.detail && Number(e.detail.verse);

    if (!Number.isFinite(verse)) return;

    openVerseExplanation(verse);
  });

  document.body.dataset.mtbExplainVerseBound = "1";
}
  }
