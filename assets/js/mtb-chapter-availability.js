(function () {
  ...
  // =========================================================
  // MTB BOOK HOME AVAILABILITY
  //
  // RULES
  // 1. Every canonical chapter is always shown because
  //    Chapter Scripture is the base content layer.
  //
  // 2. Book Overview is shown only when the actual
  //    generated Book Overview HTML exists.
  //
  // Canonical chapter counts come from window.MTB_CONTENT
  // defined in jshero-jump.js.
  // =========================================================
  ...
  // FILE VALIDATION
  //
  // Do NOT trust HTTP 200 by itself because a local/static
  // server may return a fallback page for a missing file.
  // =========================================================
  ...
  function getBookOverviewPath(book) {
    const testament =
      BOOK_TESTAMENT[book] || "new-testament";
  ...
  async function realBookOverviewExists(book) {
    const path = getBookOverviewPath(book);
    ...
    // Require actual Book Overview content.
    // This avoids false positives from a fallback HTML page.
    return (
      /mtb-doc--book-overview/i.test(html) ||
      /data-doc-type=["']book-overview["']/i.test(html) ||
      /book overview/i.test(html) &&
      html.toLowerCase().includes(book.toLowerCase())
    );
  }


  // =========================================================
  // BOOK OVERVIEW HERO BUTTON
  // =========================================================

  function findBookOverviewHeroButton() {

    // Prefer explicit selectors if available.
    ...
    // Fallback:
    // Find a button/link with visible text "Book Overview"
    // but ignore the horizontal chapter tabs.
  ...
  async function updateBookOverviewButton() {
    ...
    // Leave the current state untouched while the file check runs.
    // This prevents a valid button from flashing off and back on.
    const exists =
      await realBookOverviewExists(book);

    button.style.display = exists ? "" : "none";
    button.toggleAttribute("aria-hidden", !exists);
  }
  ...
  // CHAPTER SELECTOR
  //
  // This does NOT depend on Chapter Study availability.
  // Every canonical chapter is shown because Scripture
  // is always available as the base content.
  // =========================================================
  ...
  function renderCanonicalChapters() {
    ...
    // If this exact book/chapter set is already rendered,
    // do not tear it down and rebuild it on another popstate.
    if (
    ...
    if (
      ...
      message.className =
        "book-hero-no-chapters";

      message.textContent =
        "Chapter information is not available.";
    ...
    // Create buttons 1 through the canonical
    // chapter count for this book.
    for (
      let chapter = 1;
  ...
  function renderBookHomeAvailability() {
    ...
    // Chapters render immediately.
    renderCanonicalChapters();

    // Book Overview is independently checked.
