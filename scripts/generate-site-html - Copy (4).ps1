...
function Convert-MtbStudyScriptureToHtml([string]$text) {
  if ([string]::IsNullOrWhiteSpace($text)) { return '' }
  $normalized = ([string]$text) -replace "`r`n", "`n" -replace "`r", "`n"
  $blocks = [regex]::Split($normalized.Trim(), "`n\s*`n")
  $sb = New-Object System.Text.StringBuilder
  foreach ($block in $blocks) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($translation)) { [void]$sb.Append('<span class="mtb-study-translation">' + (ConvertTo-HtmlEncoded $translation) + '</span>') }
    [void]$sb.Append('<p>' + ((ConvertTo-HtmlEncoded $b) -replace "`n", '<br />') + '</p></blockquote>')
  }
  return $sb.ToString()
}
...
function New-MtbChapterStudyHtmlFromExcel {
  ...
  [void]$sb.Append(@'
<style>
.mtb-chapter-study{--navy:#173a5e;--blue:#2f6eae;--blue-soft:#eef4f9;--gold:#b7832f;--gold-soft:#fbf4e7;--ink:#1f2b37;--muted:#5c6873;--line:#d8e1e8;max-width:1420px;margin:0 auto;padding:2px 4px 36px;font-family:Arial,Helvetica,sans-serif;color:var(--ink)}
...
'@)
  ...
  foreach ($section in $sections) {
    ...
    [void]$sb.Append('<div class="mtb-study-block"><h2 class="mtb-study-block-title">Section Overview</h2><div class="mtb-study-grid">')
    foreach ($pair in $overviewPairs) {
      ...
      for ($c=0; $c -lt $headers.Count; $c++) {
        ...
        if ($key -match 'historical & cultural setting|historical & cultural notes|remarks') { $extra += ' mtb-study-card-wide' }
    ...
    if ($section.VerseRows.Count -gt 0) {
      ...
      foreach ($row in $section.VerseRows) {
        ...
        [void]$sb.Append('<details class="mtb-study-verse"' + $openAttr + '><summary><span>' + (ConvertTo-HtmlEncoded $reference) + '</span></summary><div class="mtb-study-verse-body">')
        [void]$sb.Append((Convert-MtbStudyScriptureToHtml $scripture))
